...
import graphql_connectors

from graphql_scalars import custom_scalars
...
async def resolve_book(*_, id):
    return await graphql_connectors.get_book_by_id(id)
...
async def resolve_lend(*_, id):
    return await graphql_connectors.get_lend_by_id(id)

@query.field('lends')
async def resolve_lends(*_, **arguments):
    return await graphql_connectors.get_lends(arguments)

@mutation.field('addLend')
async def resolve_add_lend(_, info, input):
    return await graphql_connectors.add_lend(input)

@mutation.field('updateLend')
async def resolve_update_lend(_, info, id, input):
    return await graphql_connectors.update_lend(id, input)

@mutation.field('deleteLend')
async def resolve_delete_lend(_, info, id):
    return await graphql_connectors.delete_lend(id)

lend = ObjectType('Lend')

@lend.field('book')
async def resolve_field_book(obj, info):
    return await graphql_connectors.get_book_by_id(str(obj['bookId']))

@lend.field('customer')
async def resolve_field_customer(obj, info):
    return await graphql_connectors.get_customer_by_id(obj['customerId'])
